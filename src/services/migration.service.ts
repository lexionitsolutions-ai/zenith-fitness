  ...
  try {
    await prisma.$transaction(async (transaction) => {
      await transaction.$executeRaw`
        ...
        INSERT INTO "Member" ("admissionId", "fullName", "mobileNumber", "originalMobileNumber", "gender", "ageAtImport", "birthDate", "address", "medicalHistory", "sourceSheet", "sourceRow")
        SELECT row->>'admissionId', row->>'fullName', NULLIF(row->>'mobileNumber',''), NULLIF(row->>'originalMobileNumber',''), NULLIF(row->>'gender',''), NULLIF(row->>'ageAtImport','')::int, NULLIF(row->>'birthDate','')::date, NULLIF(row->>'address',''), NULLIF(row->>'medicalHistory',''), row->>'sourceSheet', (row->>'sourceRow')::int FROM latest
        ON CONFLICT ("admissionId") DO UPDATE SET "fullName" = EXCLUDED."fullName", "mobileNumber" = COALESCE(EXCLUDED."mobileNumber", "Member"."mobileNumber"), "originalMobileNumber" = COALESCE(EXCLUDED."originalMobileNumber", "Member"."originalMobileNumber"), "gender" = COALESCE(EXCLUDED."gender", "Member"."gender"), "ageAtImport" = COALESCE(EXCLUDED."ageAtImport", "Member"."ageAtImport"), "birthDate" = COALESCE(EXCLUDED."birthDate", "Member"."birthDate"), "address" = COALESCE(EXCLUDED."address", "Member"."address"), "medicalHistory" = COALESCE(EXCLUDED."medicalHistory", "Member"."medicalHistory"), "sourceSheet" = EXCLUDED."sourceSheet", "sourceRow" = EXCLUDED."sourceRow"`;
      await transaction.$executeRaw`
        WITH rows AS (SELECT value AS row FROM jsonb_array_elements(${payload}::jsonb)), latest AS (
        ...
        UPDATE "User" AS user_record SET "pinHash" = ${pinHash}, "isActive" = true, "failedLoginCount" = 0, "lockedUntil" = NULL
      ...
      await transaction.$executeRaw`
        ...
        INSERT INTO "User" ("memberId", "mobileNumber", "pinHash", "role", "mustChangePassword")
        SELECT member_record.id, latest.row->>'mobileNumber', ${pinHash}, 'MEMBER'::"Role", true FROM "Member" AS member_record JOIN latest ON latest.row->>'admissionId' = member_record."admissionId"
      ...
      await transaction.$executeRaw`
        ...
        INSERT INTO "Membership" ("memberId", "planId", "planDays", "category", "startDate", "endDate", "totalAmount", "discountAmount", "finalAmount", "amountPaid", "pendingAmount", "paymentMode", "paymentStatus", "sourcePaymentStatus", "membershipStatus", "remarks", "sourceSheet", "sourceRow", "sourceType", "sourceTimestamp")
        SELECT member_record.id, plan_record.id, NULLIF(row->>'planDays','')::int, NULLIF(row->>'category',''), NULLIF(row->>'startDate','')::date, NULLIF(row->>'endDate','')::date, (row->>'totalAmount')::numeric, (row->>'discountAmount')::numeric, (row->>'finalAmount')::numeric, (row->>'amountPaid')::numeric, (row->>'pendingAmount')::numeric, NULLIF(row->>'paymentMode',''), (row->>'paymentStatus')::"PaymentStatus", NULLIF(row->>'sourcePaymentStatus',''), (row->>'membershipStatus')::"MembershipStatus", NULLIF(row->>'remarks',''), row->>'sourceSheet', (row->>'sourceRow')::int, 'LEGACY_IMPORT'::"SourceType", NULLIF(row->>'sourceTimestamp','')::timestamp
